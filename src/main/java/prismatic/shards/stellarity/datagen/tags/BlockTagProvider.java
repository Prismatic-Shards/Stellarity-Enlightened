...
public class BlockTagProvider extends FabricTagsProvider.BlockTagsProvider {
	...
	protected void addTags(HolderLookup.@NonNull Provider provider) {
		...
		addTags(WORLDGEN_REPLACEABLE_STALACTITE, BlockTags.TERRACOTTA, BASE_STONE_NETHER, BASE_STONE_OVERWORLD, StellarityBlockTags.DIRT).add(END_STONE, DIRT, TUFF, BASALT, MOSS_BLOCK, SMOOTH_BASALT, DEEPSLATE, COBBLED_DEEPSLATE, GRASS_BLOCK, SCULK, PACKED_MUD, SMOOTH_SANDSTONE, SNOW_BLOCK, Blocks.ICE, WHITE_CONCRETE_POWDER, CALCITE, MOSS_BLOCK, WARPED_WART_BLOCK);
		addTags(ANIMALS_SPAWNABLE_ON).add(ENDER_GRASS_BLOCK);
		addTags(WORLDGEN_REPLACEABLE_END_STONE).add(END_STONE);
		addTags(WORLDGEN_REPLACEABLE_AMETHYST_FOREST_BOTTOM).add(END_STONE, CALCITE, TUFF, GRASS_BLOCK, DIRT, ENDER_DIRT, ENDER_GRASS_BLOCK, DIORITE);
		...
		addTags(WORLDGEN_REPLACEABLE_CARVER, WORLDGEN_REPLACEABLE_CARVER);
	}
}
