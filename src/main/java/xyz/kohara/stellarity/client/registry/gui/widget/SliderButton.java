...
public class SliderButton extends AbstractSliderButton {
	...
	public SliderButton(double min, double max, Function<Double, Component> title, Function<Double, @Nullable Tooltip> tooltip, double d, Consumer<Double> onValueChange) {
		this(min, max, 310, 20, title, tooltip, d, onValueChange);
	}


	public SliderButton(double min, double max, int k, int l, Function<Double, Component> message, Function<Double, @Nullable Tooltip> tooltip, double d, Consumer<Double> onValueChange) {
		super(0, 0, k, l, message.apply(d), (d - min) / (max - min));
	...
	protected void setValue(double d) {
		...
		if (e != this.value) {
			onValueChange.accept(scaledValue);
		}
